#ifndef _LED_EFFECT_FUNCTIONS_
#define _LED_EFFECT_FUNCTIONS_
...
//-------------------------------newKITT---------------------------------------
void rainbowCycle(int SpeedDelay) {
  if ((millis() - last_ticks_millis) < SpeedDelay)
  {
    return;
  }
  last_ticks_millis = millis();
  byte *c;
  for (uint16_t i = 0; i < LED_COUNT; i++) {
    c = Wheel(((i * 256 / LED_COUNT) + rainbow_index) & 255);
